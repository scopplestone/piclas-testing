...
! Private Part ---------------------------------------------------------------------------------------------------------------------
! Public Part ----------------------------------------------------------------------------------------------------------------------
PUBLIC :: ElectronicEnergyExchange, InitElectronShell, TVEEnergyExchange, ReadSpeciesLevel, CalcProbCorrFactorElec!, ReadSpeciesVibronicLevel
...
CASE(1)
  ...
  DO WHILE ( iRan2 .GE. gtemp / gmax )
    ...
    CALL RANDOM_NUMBER(iRan2)
  END DO
  PartIntEn(iPart1)%EElec = BoltzmannConst * SpecDSMC(iSpec)%ElectronicState(2,iQua)
CASE(2)
...
CASE(3)
  ...
  IF(PRESENT(XSec_Level))THEN
    IF(XSec_Level.GT.0) THEN
      ...
      PartIntEn(iPart1)%EElec = SpecXSec(iCase)%ElecLevel(XSec_Level)%Threshold
    ELSE
...
USE MOD_DSMC_Vars               ,ONLY: SpecDSMC, PartIntEn, CollInf, ElecRelaxPart
...
DO iLoop = 1, nPart
  ...
  IF((Species(iSpec)%InterID.NE.4).AND.(.NOT.SpecDSMC(iSpec)%FullyIonized)) THEN
    ...
    IF ((1.-ElecExpSpec(iSpec)).GT.iRan) THEN
      ...
      OldEn = OldEn + PartIntEn(iPartIndx_NodeRelaxElec(nElecRelax))%EElec(1)* partWeight
    END IF
  END IF
END DO
...
DO iLoop = 1, nElecRelax
  ...
  IF (TEqui.GT.SpecDSMC(iSpec)%MaxMeanXiElec(2)) THEN
    ...
    IF ((OldEn - (PartIntEn(iPart)%EElec(1)*partWeight)).LT.0.0) THEN
      DO WHILE ((OldEn - (PartIntEn(iPart)%EElec(1)*partWeight)).LT.0.0)
        iQua = iQua - 1
        PartIntEn(iPart)%EElec = BoltzmannConst*SpecDSMC(iSpec)%ElectronicState(2,iQua)
        IF (iQua.EQ.0) EXIT
      END DO
  ...
  ELSE
    NewEnElec(iSpec) = NewEnElec(iSpec) + PartIntEn(iPart)%EElec(1) * partWeight
  END IF
END DO
...
IF (ANY(NewEnElec.GT.0.0)) THEN
  ...
  DO iLoop = 1, nElecRelax
    ...
    iSpec = PartSpecies(iPart)
    IF (SkipEnergyCons(iSpec)) CYCLE
    betaV = alpha(iSpec)*PartIntEn(iPart)%EElec(1)
    ...
    IF (iRan.GT.Prob) THEN
      iQuant = iQuaMax -1
      PartIntEn(iPart)%EElec = BoltzmannConst*SpecDSMC(iSpec)%ElectronicState(2,iQuaMax-1)
    ELSE
      iQuant = iQuaMax
      PartIntEn(iPart)%EElec = BoltzmannConst*SpecDSMC(iSpec)%ElectronicState(2,iQuaMax)
    END IF
    IF ((OldEn - (PartIntEn(iPart)%EElec(1)*partWeight)).LT.0.0) THEN
      DO WHILE ((OldEn - (PartIntEn(iPart)%EElec(1)*partWeight)).LT.0.0)
        iQuant = iQuant - 1
        PartIntEn(iPart)%EElec = BoltzmannConst*SpecDSMC(iSpec)%ElectronicState(2,iQuant)
        IF (iQuant.EQ.0) EXIT
      END DO
    END IF
    OldEn = OldEn - PartIntEn(iPart)%EElec(1)*partWeight
...
!===================================================================================================================================
USE MOD_DSMC_Vars             ,ONLY: DSMC, SpecDSMC, PartIntEn
USE MOD_Particle_Vars         ,ONLY: PartSpecies
USE MOD_Globals_Vars          ,ONLY: BoltzmannConst
! IMPLICIT VARIABLE HANDLING
IMPLICIT NONE
...
PartIntEn(iPart1)%EElec = BoltzmannConst * SpecDSMC(PartSpecies(iPart1))%ElectronicState(2,iQua)
PartIntEn(iPart1)%EVib = (jQVib + DSMC%GammaQuant) * BoltzmannConst &
...
!SUBROUTINE ReadSpeciesVibronicLevel ( Dsetname, iSpec )
!!===================================================================================================================================
!! Subroutine to read the electronic levels from DSMCSpeciesElectronicState.h5
!!===================================================================================================================================
!! use module
...
!!  ElLevelDatabase = TRIM(SpeciesDatabase)
!!END IF
!LBWRITE(UNIT_StdOut,'(A)') ' | Read vibronic level entries '//TRIM(datasetname)//' from '//TRIM(ElLevelDatabase)
!! Initialize FORTRAN interface.
!CALL H5OPEN_F(err)
!! Open the file.
!CALL H5FOPEN_F (TRIM(ElLevelDatabase), H5F_ACC_RDONLY_F, file_id_dsmc, err)
...
!print* dims, SizeMax
!! Allocate electronic_state
!!ALLOCATE (VibronicState( 1:dims(1), 0:dims(2)-1 ) )
!!! read data
!!CALL H5dread_f(dset_id_dsmc, H5T_NATIVE_DOUBLE, ElectronicState, dims, err)
!!ALLOCATE ( SpecDSMC(iSpec)%ElectronicState( 1:dims(1), 0:dims(2)-1 ) )
!!SpecDSMC(iSpec)%ElectronicState = ElectronicState
...
!END SUBROUTINE ReadSpeciesVibronicLevel
